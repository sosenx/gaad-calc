...
* 
*/
class spiral_bound_catalog extends catalog {
	...
	function set_base_defaults( ){
		...
		if ( empty( $this->base ) || is_null( $this->base ) ) {
			$this->base = array(
				'post_title' => __( 'Spiral bound catalog', 'gcalc' ),
...
/**
	...
	public static function get_attr_defaults(  ){
		$r = array( 
				...
				array( 'cover_type', array( 'spiral_binding' ), '111' ),
	...
	public function set_attr_defaults(  ){
		...
		if ( empty( $this->attr ) || is_null( $this->attr ) ) {
			$this->attr = \gcalc\db\product\spiral_bound_catalog::get_attr_defaults();
		}
	}
